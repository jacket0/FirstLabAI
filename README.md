...
|--------|----------|-----------|--------|----------|
| Pretrained + Adam | [Результат будет после запуска] | [Результат будет после запуска] | [Результат будет после запуска] | [Результат будет после запуска] |
| Pretrained + RAdam | [Результат будет после запуска] | [Результат будет после запуска] | [Результат будет после запуска] | [Результат будет после запуска] |
| Scratch + Adam | [Результат будет после запуска] | [Результат будет после запуска] | [Результат будет после запуска] | [Результат будет после запуска] |

*Примечание: Результаты будут заполнены после выполнения обучения. Для получения результатов запустите скрипт `lab_regnet_dogs_kaggle.py`.*
...
1. **Дообучение vs Обучение с нуля**:
   - Предобученные модели должны показать значительно лучшие результаты
   - Обучение с нуля требует больше эпох и данных для достижения сопоставимого качества

2. **Adam vs RAdam**:
   - RAdam может показать более стабильное обучение
   - Разница в финальных метриках может быть незначительной, но RAdam часто обеспечивает более плавную сходимость

3. **Метрики Precision и Recall**:
   - Высокий Precision означает, что модель редко ошибается при предсказании класса
   - Высокий Recall означает, что модель находит большинство примеров каждого класса
   - Баланс между этими метриками важен для практического применения
...
2. **RegNet Architecture**:
   - Radosavovic, I., Kosaraju, R. P., Girshick, R., He, K., & Dollár, P. (2020). Designing network design spaces. Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR).
